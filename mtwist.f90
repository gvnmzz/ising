...
! Period parameters
    integer, parameter :: NN = 624, N1 = NN + 1
...
! Period parameters
    integer, parameter :: MM = 397, MATA  = -1727483681
!                                    constant vector a
...
!                              a default initial seed is used
      ...
      do kk=0,NN-MM-1
          ...
          mt(kk)=ieor(ieor(mt(kk+MM),ishft(y,-1)),mag01(iand(y,1)))
      enddo
      do kk=NN-MM,NN-2
          y=ior(iand(mt(kk),UMASK),iand(mt(kk+1),LMASK))
          mt(kk)=ieor(ieor(mt(kk+(MM-NN)),ishft(y,-1)),mag01(iand(y,1)))
      enddo
      y=ior(iand(mt(NN-1),UMASK),iand(mt(0),LMASK))
      mt(NN-1)=ieor(ieor(mt(MM-1),ishft(y,-1)),mag01(iand(y,1)))
      mti = 0
    endif
